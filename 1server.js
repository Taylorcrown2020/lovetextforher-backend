...
// ============================================================
// ADMIN LOGIN — FULLY FIXED FOR LOCALHOST
// ============================================================
app.post("/api/admin/login", async (req, res) => {
    try {
        ...
        res.cookie("admin_token", token, {
            httpOnly: true,
            secure: false,   // CRITICAL — local fix
            sameSite: "lax",
            path: "/",
            maxAge: 7 * 24 * 60 * 60 * 1000
        });
...
app.get("/api/admin/kpis", authAdmin, async (req, res) => {
    try {
        ...
        const activeRecipients = await pool.query(`SELECT COUNT(*) FROM users WHERE is_active=true`);
        const unsubscribed = await pool.query(`SELECT COUNT(*) FROM users WHERE is_active=false`);

        const activeSubs = 0;          // placeholder (you haven't added Stripe subs yet)
        const mrr = 0;                 // placeholder
        const churn = 0;               // placeholder

        const totalCarts = await pool.query(`SELECT COUNT(*) FROM carts`);
        const activeCarts = await pool.query(`SELECT COUNT(*) FROM carts WHERE json_array_length(items) > 0`);
        
        const avgCartValue = 0;        // placeholder  
        const recoverableRevenue = 0;  // placeholder

        res.json({
            customers: {
                total: Number(customers.rows[0].count)
            },
            recipients: {
                ...
                netGrowth: Number(activeRecipients.rows[0].count) - Number(unsubscribed.rows[0].count)
            },
            subscriptions: {
                active: activeSubs,
                mrr: mrr,
...
// ============================================================
// STRIPE PLACEHOLDER — ADD YOUR STRIPE CODE HERE
// ============================================================
console.log("⚠️ Stripe not connected — insert your checkout code HERE");
